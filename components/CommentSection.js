...
export default function CommentSection({ entryId }) {
  ...
  const renderComment = (c, isReply = false, parentId = null) => {
    ...
    return (
      <div key={c.id} className={`relative group transition-all duration-300 mb-4 ${isReply ? "ml-8 mt-2 pl-4 border-l-2 border-accent/20" : "p-5 rounded-xl border border-black/5 dark:border-white/5 bg-white/40 dark:bg-white/5 hover:bg-white/60 dark:hover:bg-white/10"} ${isPinned ? "ring-2 ring-accent/50 bg-accent/5" : ""}`}>
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="text-xs font-bold uppercase tracking-widest text-muted/80">{c.authorName}</div>
          <div className="text-[10px] text-muted/50">{c.timestamp?.toDate().toLocaleDateString()}</div>
        </div>
        ...
        <div className="flex gap-4 mt-3 text-xs font-bold text-muted/60 opacity-60 group-hover:opacity-100 transition-opacity">
          {!isReply && user && (
            <button onClick={() => setReplyBoxes(p => ({ ...p, [c.id]: !p[c.id] }))} className="hover:text-accent transition-colors flex items-center gap-1"><FiCornerDownRight /> Reply</button>
          )}
          {isOwner && (
            <button onClick={() => handleDelete(isReply ? ["entries", entryId, "comments", parentId, "replies", c.id] : ["entries", entryId, "comments", c.id])} className="hover:text-red-500 transition-colors flex items-center gap-1"><FiTrash2 /> Delete</button>
          )}
          {isAdmin && !isReply && (
            <button onClick={() => pinComment(isPinned ? null : c.id)} className="hover:text-accent transition-colors flex items-center gap-1"><FiMapPin /> {isPinned ? "Unpin" : "Pin"}</button>
          )}
        </div>
        {!isReply && replyBoxes[c.id] && (
          <div className="mt-3 flex gap-2 animate-in fade-in slide-in-from-top-1">
            <input className="text-sm w-full p-2 bg-white/50 dark:bg-black/20 rounded border border-black/5" placeholder="Whisper back..." onKeyDown={(e) => e.key === 'Enter' && handleReplySubmit(c.id, e.target.value)} />
          </div>
        )}
